...
- **根治 `docker inspect` 看到的版本号长期停在旧值的问题**：Dockerfile 里的 `version` 标签一直是硬编码，从 1.7.10 之后发布的多个版本都忘了同步更新它，导致用户用 `docker inspect ... Config.Labels.version` 查看时永远显示 `1.7.10`，误以为 `docker compose pull` 没有拉到新版本（实际镜像内容是最新的，只是这个标签在“撒谎”）。此前 v1.7.4 也踩过同一个坑、当时只是手动补了个新数字，没有根治，所以又复发了。
- **现在版本号由 CI 构建时自动注入，不再需要人工维护**：用固定 tag（如 `:1.8.4`）的用户会看到精确版本号；用 `:latest` 的用户会看到形如 `v1.8.4-3-gabc123` 的描述串（最近版本 + 领先提交数 + 提交号），可据此判断是否已更新到最新，不会再被一个固定不变的数字误导。
