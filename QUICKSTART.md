...
### 本项目是什么？
TeslaMate 官方的 Grafana 图表是英文的。本项目提供了 **45 个简体中文汉化版图表**（含 13 个原创分析仪表盘），把所有界面翻译成中文，开箱即用。
...
- 🆕 **13 个原创分析仪表盘** —— 年度驾驶报告、省钱分析、充电健康管理、⚡ 行车 vs 停车耗电（月度）、出行规律分析、动能回收分析、续航退化分析、驾驶评分、多车对比、⚡ 分时电价配置、🏆 充电桩性价比榜、🌡️ 天气-能耗关联、胎压
...
> 💡 **准备用下面「方法 A：一键脚本」的 Linux 用户可以跳过本关**：`simple-deploy.sh` 在主流 Linux 发行版（Ubuntu/Debian/CentOS 等）上检测到没装 Docker 时会交互式询问是否自动安装；非交互环境（无人值守/脚本调用）加环境变量 `AUTO_INSTALL_DOCKER=1` 跳过询问直接装。**macOS / 群晖 NAS 用户仍需按下方步骤预先手动装好 Docker**——这两个平台脚本不会自动安装。
...
> 方法 A 是脚本帮你做完所有配置；方法 B 给已经熟 Docker、想改 ports / volumes / 路径的人用；方法 C 只面向已有 Grafana 的进阶用户。

<a id="one-click-install"></a>
...
- **自动安装四个 SQL 安装文件**（坐标转换 / 单位换算 / 分时电价 / 性能索引）
- 在终端显示 Grafana 密码、数据库密码和 `ENCRYPTION_KEY`，请全部存入密码管理器
...
> **🇨🇳 中国大陆用户镜像加速（如果脚本卡在 `Pulling image...`）：**脚本默认使用 Docker Hub 镜像 `bswlhbhmt816/teslamate-chinese-dashboards`。若仍然很慢或失败，按 [镜像拉取失败修复](TROUBLESHOOTING.md#image-pull-cn) 选择 Docker Hub、镜像代理、网络代理或离线导入；镜像源配置只在该修复章节维护。

---

<a id="manual-compose"></a>
...
Grafana 镜像不会自动把函数装入 PostgreSQL。手动 Compose 用户必须执行 [四个 SQL 安装文件的权威循环](TROUBLESHOOTING.md#repair-sql-install)：它会自动探测 database 容器，依次安装坐标转换、单位换算、分时电价和性能索引，任一文件失败即停止。

---

<a id="manual-dashboard-mount"></a>

### 方法 C：在已有 Grafana 中手动挂载仪表盘（进阶）

适合需要完全控制 `docker-compose.yml`、只想把现有 Grafana 换成中文仪表盘的用户。部分仪表盘使用 `schemaVersion 41`，需要 **Grafana 12+**（TeslaMate Grafana 镜像 3.0.0+）。
...
`internal/` 必须挂载到 `/dashboards_internal/`（带下划线），否则「行程详情」和「充电详情」仍显示英文。然后执行 [四个 SQL 安装文件的权威循环](TROUBLESHOOTING.md#repair-sql-install)；手动挂载也需要坐标转换、单位换算、分时电价和性能索引。
...
如果新版包含 SQL 变更，再执行同一个 [四个 SQL 安装文件的权威循环](TROUBLESHOOTING.md#repair-sql-install)。
...
- **方法 A 一键脚本**：用户名是 `admin`，密码是脚本结尾显示的随机密码。也可在 `~/teslamate-chinese/docker-compose.yml` 中查找 `GF_SECURITY_ADMIN_PASSWORD`。
- **方法 B 手动 Compose**：只有在未设置 `GF_SECURITY_ADMIN_PASSWORD` 时，首次登录才是 `admin / admin`。

> ⚠️ 手动 Compose 用户如果仍是 `admin / admin`，首次登录后立即到右上角头像 → Profile → Change Password 修改。一键脚本用户请把随机密码保存到密码管理器。

### 界面说明

登录后你会看到左侧导航栏，点击 **仪表盘（Dashboards）** 查看所有 45 个中文图表。这里保留 `Dashboards` 是为了对应 Grafana 界面中的菜单文字。
...
- **「驾驶评分」** — 急加速 / 急刹阈值 0.3g、急转阈值 0.4g，按超阈时间占比评分，采样间隔最多计 60 秒
- **「🌡️ 天气-能耗关联」** — 量化温度对续航的影响（国内冬天痛点）
...
**Q: 怎么更新到新版本？**
A: 详见 [README → ⚡ 升级到最新版](README.md#upgrade-latest) 的方法 A/B/C/D（按你之前怎么装的选）。
...
**老版本升级**：按原安装方式执行 [README → 升级到最新版](README.md#upgrade-latest)。**手动验证或修复**：执行 [四个 SQL 安装文件的权威循环](TROUBLESHOOTING.md#repair-sql-install)。
...
`sql/install-coord-functions.sql` 装了 5 个函数：

```sql
is_outside_china(lat, lng)      -- 判断坐标是否在中国境内 bbox 之外，境外直接短路不转换
wgs84_to_gcj02_lat(lat, lng)    -- 算法实现（WGS-84 → GCJ-02，单向，无反向函数）
...
> **先决条件**：你的 TeslaMate 已经升到 v1.5.0+（v1.8.0 当前最新）。还在 v1.4.x 的用户先按 [README → 升级到最新版](README.md#upgrade-latest) 升级；新装用户直接看第 2 步。

#### 1. 升级（仅 v1.4.x 老用户）

升级命令只在 [README → 升级到最新版](README.md#upgrade-latest) 维护。按原安装方式选择一键脚本、git clone、自写 Compose 或官方版迁移，不要混用。
...
按提示选：
1. 你城市（北京/上海/深圳/广州/浙江/江苏/武汉 共 7 个内置模板）
2. 应用到哪个收藏点（Geofence，数据库中称地理围栏；例如你在 TeslaMate 里设置的「家」）
3. 是否按账单微调单价（按你电费账单实际数字改）
4. 快充是否单独算（一般跳过，超充按运营商 App 真实付款最准）
5. 试算最近一笔家充对账

或者打开 Grafana 的「**⚡ 分时电价配置**」仪表盘 →「**🌆 一键导入城市模板**」一键套用。写入、删除和历史重算等写操作在提交前都会二次确认；「**全局默认**」用于没有收藏点或缺少位置的充电记录。
...
- **充电费用统计 / 省钱分析 / 年度驾驶报告** — 9 个仪表盘 60+ 处 SQL 自动按分时显示
...
**Grafana（`:3000`）的密码取决于安装方式。** 一键脚本使用脚本结尾显示的随机密码（也可查 `docker-compose.yml` 的 `GF_SECURITY_ADMIN_PASSWORD`）；手动 Compose 未设该变量时才是 `admin / admin`。如果仍用 `admin / admin`，陌生人扫到 IP 就能登入看完整数据。

**这就是为什么不能直接把端口暴露到公网。** 下面 5 级防护清单按优先级做。

### 🔴 必做 1：确认 Grafana 使用强密码

一键脚本用户已获得强随机密码，保存到密码管理器即可。手动 Compose 用户如果仍是 `admin / admin`，第一次登录 Grafana 后**立即**修改：

```
Grafana → 右上角头像 → Profile → Change Password
```

新密码至少 12 位，含大小写 + 数字。**继续使用 `admin / admin` 时，其他防护都白搭。**
...
| 文档 | 内容 |
|------|------|
| [SCENE_GUIDE.md](SCENE_GUIDE.md) | 什么场景看什么仪表盘 |
| [DASHBOARD_MAP.md](DASHBOARD_MAP.md) | 45 个仪表盘导航地图 |
